...
int TickFct_LS(int state){
 switch(state){
  case start:
    ...
    break;
  case lock_r:
    state = (digitalRead(A0) && digitalRead(A3) && !digitalRead(A1) && !digitalRead(A2)) ? unlock_p : lock_r;
    break;
  case unlock_p:
    myServo.write(0);
    state = (digitalRead(A0) && digitalRead(A3) && !digitalRead(A1) && !digitalRead(A2)) ? unlock_p : unlock_r;
    break;
  case unlock_r:
    state = (digitalRead(A0) && digitalRead(A3) && !digitalRead(A1) && !digitalRead(A2)) ? lock_p : unlock_r;
    break;
  case lock_p:
    state = (digitalRead(A0) && digitalRead(A3) && !digitalRead(A1) && !digitalRead(A2)) ? lock_p : start;
    break;
 }
  return state;
}
